...
trans<- read.transactions("ByProductType.csv", format = "basket", sep=";", rm.duplicates = TRUE)
...
itemFrequencyPlot(trans[one], support=0.05, cex.names=0.8)
...
freq.itemsets <- eclat(trans, parameter=list(supp=0.05, maxlen=30))
inspect(sort(freq.itemsets, by="support"))


#rules
rules <- apriori(trans, parameter = list(support = 0.01, confidence = 0.25))
inspect(sort(sort(rules, by="support"), by="confidence")[1:25])
